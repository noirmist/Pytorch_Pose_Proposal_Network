...
for idx, anno in enumerate(c["annotations"]):
    ...
    # change bbox x,y from top left to center
    x, y, width, height = anno['bbox']
    new_anno['bbox'] = [ x+width//2, y+height//2, width, height]
    ...
    if left_shoulder_v >= 1 and right_shoulder_v >=1:
        thorax = (left_shoulder + right_shoulder) / 2.
        labeled = 1
        key = np.vstack([key, np.array([*thorax, labeled])])
        n += 1
    else:
        labeled = 0
        #dummy data
    ...
    thorax, thorax_v = key[thorax_idx][:2], key[thorax_idx][2]
    nose, nose_v = key[nose_idx][:2], key[nose_idx][2]
    
    # neck
    if thorax_v >=1 and nose_v >=1:
        neck = (thorax + nose) / 2.
        labeled = 4
...
for idx, anno in enumerate(m):
    ...
    width = x_axis.max()-x_axis.min() + w
    height = y_axis.max()-y_axis.min() + h
