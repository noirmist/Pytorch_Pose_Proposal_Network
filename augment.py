...
def augment_image(image, dataset_type):
    ...
    if method == 'random_distort':
        ...
        ret = ts.random_distort(image, contrast_low=0.3, contrast_high=2)
        return ret, param
    if method == 'pil':
...
def random_flip(image, keypoints, bbox, is_labeled, is_visible, flip_indices):
    ...
    """
    _, H, W = image.shape
    image, param = ts.random_flip(image, x_random=True, return_param=True)

    if param['x_flip']:
        keypoints = [
            ts.flip_point(points, (H, W), x_flip=True)[flip_indices]
            for points in keypoints
        ]
        ...
        new_bbox = []
        for x, y, w, h in bbox:
            [[y, x]] = ts.flip_point(np.array([[y, x + w]]), (H, W), x_flip=True)
...
def scale_fit_short(image, keypoints, bbox, length):
    ...
    min_hw = min(H, W)
    scale = length / min_hw
    new_image = ts.scale(image, size=length, fit_short=True)
...
def random_sized_crop(image, keypoints, bbox):
    image, param = ts.random_sized_crop(
        image,
...
def resize(image, keypoints, bbox, size):
    _, H, W = image.shape
    new_h, new_w = size
    image = ts.resize(image, (new_h, new_w))

    keypoints = [
        ts.resize_point(points, (H, W), (new_h, new_w))
        for points in keypoints
    ]

    new_bbox = []
    for x, y, bw, bh in bbox:
        [[y, x]] = ts.resize_point(np.array([[y, x]]), (H, W), (new_h, new_w))
        bw *= new_w / W
        bh *= new_h / H
